...
Schema 2 menyimpan snapshot `job`, riwayat `job_event` append-only, dan `audit_event` terkait dalam transaksi yang sama. State machine menggunakan optimistic revision/CAS, termasuk antar-handle `JobStore` dalam proses yang sama; progress hanya dapat berubah pada `RUNNING`/`CANCELLING`; permintaan cancellation bersifat kooperatif dan idempotent; recovery restart mengubah pekerjaan aktif menjadi `FAILED` dengan kode `INTERRUPTED` yang retriable. Setiap descriptor hasil baca divalidasi kembali sebagai data tidak tepercaya sebelum digunakan atau dikembalikan. Snapshot dan history dapat dibuka kembali setelah restart.
