...
| Schema migration interrupted between database and manifest publication | explicit schema-1-to-2 upgrade only; synced byte-for-byte backups plus bounded recovery marker block normal open; failure restores both control files and proves identity/integrity/version/audit chain, otherwise leaves recovery-required evidence intact |
...
| Lost concurrent job update | positive `expected_revision`, `BEGIN IMMEDIATE`, and `UPDATE ... WHERE revision = expected_revision`; stale CAS rolls back without snapshot, job event, or audit event changes |
