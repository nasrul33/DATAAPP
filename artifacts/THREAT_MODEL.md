...
| Lost concurrent job update | process-local pinned identities are shared across independent `JobStore` handles; positive `expected_revision`, `BEGIN IMMEDIATE`, and `UPDATE ... WHERE revision = expected_revision` make stale peers reach `RevisionConflict` without partial snapshot/event writes |
